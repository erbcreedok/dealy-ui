...
export const Typography = styled(MuiTypography)<{ inline?: boolean; nowrap?: boolean; ellipsis?: boolean; textDecoration?: string }>`
  ${({ inline }) => inline && css`display: inline`}
  ${({ nowrap }) => nowrap && css`white-space: nowrap`}
  ${({ ellipsis }) => ellipsis && css`text-overflow: ellipsis; overflow: hidden; max-width: 100%`}
