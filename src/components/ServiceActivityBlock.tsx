...
import { ServiceActivity, ServiceMultiActivity } from 'src/types/ActivityTypes'
import { getDurationString } from 'src/utils/getDurationString'
import { Flex } from 'src/components/Flex'
import { Tag } from 'src/components/styled/CalendarComponents'
...
export const ActivityBlockTags: FC<ActivityBlockTagsProps> = ({
	serviceType,
	price,
	duration,
}) => (
...
export const ActivityBlockActionBar: FC<ActivityBlockActionBarProps> = ({
	isCanceled,
	onActionClick,
	onPhoneClick,
	phone,
	...
	<Flex ml="auto" alignItems="flex-start" flexShrink={0}>
		...
		<IconButton
			size="small"
			sx={{ color: isCanceled ? COLORS.red : COLORS.grey }}
...
export const ServiceActivityBlock: FC<ServiceActivityBlockProps> = ({
	activity,
}) => {
	...
	return (
		<Flex px="8px" py="4px" flexGrow={1} maxWidth="100%">
			<Box flexShrink={1} maxWidth="calc(100% - 76px)">
				...
				>
					<Hint mr={1} inline color={isCanceled ? COLORS.red : undefined}>
						{activity.clientTag}
					</Hint>
...
export const ServiceMultiActivityBlock: FC<ServiceMultiActivityBlockProps> = ({
	activity,
}) => {
	const isCanceled = useMemo(() => activity.canceled, [activity.canceled])
	...
	return (
		<Flex px="8px" py="4px" flexGrow={1} maxWidth="100%">
			<Box flexShrink={1} maxWidth="calc(100% - 76px)">
				...
				>
					<Typography inline nowrap>
						{activity.peopleCount} человека{' '}
					</Typography>
					<Hint mr={1} inline color={isCanceled ? COLORS.red : undefined}>
						из {activity.totalCount}
					</Hint>
				</Typography>
				<ActivityBlockTags
					serviceType={activity.serviceType}
